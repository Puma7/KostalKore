...
async def async_setup_entry(
    ...
    for description in SWITCH_SETTINGS_DATA:
        if description.module_id not in available_settings_data or description.key not in (
    ...
    try:
        string_count = int(string_count_setting["devices:local"]["Properties:StringCnt"])
    except ValueError:
        string_count = 0

    dc_strings = tuple(range(string_count))
    dc_string_feature_ids = tuple(
        PlenticoreShadowMgmtSwitch.DC_STRING_FEATURE_DATA_ID % dc_string for dc_string in dc_strings
    )

    dc_string_features = await plenticore.client.get_setting_values(
        {PlenticoreShadowMgmtSwitch.MODULE_ID: dc_string_feature_ids}
    )

    for dc_string, dc_string_feature_id in zip(dc_strings, dc_string_feature_ids, strict=True):
        try:
            dc_string_feature = int(
                dc_string_features[PlenticoreShadowMgmtSwitch.MODULE_ID][dc_string_feature_id]
            )
        except ValueError:
            dc_string_feature = 0
...
class PlenticoreDataSwitch(CoordinatorEntity[SettingDataUpdateCoordinator], SwitchEntity):
...
class PlenticoreShadowMgmtSwitch(CoordinatorEntity[SettingDataUpdateCoordinator], SwitchEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{entry_id}_{self.MODULE_ID}_{self.SHADOW_DATA_ID}_{dc_string}"
        self._attr_device_info = device_info
