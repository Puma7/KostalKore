...
When OFF: REG 1038 (bat_max_charge_limit) = 20000 → normal operation
...
class BatteryChargeBlockSwitch(SwitchEntity):
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        """Restore normal charging: REG 1038 = 20000."""
        self._is_on = False
        self._cancel_keepalive()
        await self._write_normal()
        self.async_write_ha_state()
        _LOGGER.info("Battery charging RESTORED (REG 1038 = 20000)")
    ...
    async def _write_normal(self) -> None:
        ...
        if reg:
            try:
                await self._coord.async_write_register(reg, 20000.0)
            except Exception as err:
